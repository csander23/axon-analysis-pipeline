"""
Configuration Manager for branch-based-snakes.py
...
class AnalysisConfig:
    """Configuration class for branch-based-snakes analysis"""
    ...
    regression_model_path: str = "/Users/charlessander/Desktop/patzke lab computing/Sudhof Lab/Axon Analysis Pipeline/Confocal_Axon_Analysis_Pipeline/Active_Model_and_Configuration_Directory/active_model.json"
    ...
    conditions: List[str] = field(default_factory=lambda: ["WT", "KO", "E2", "E4"])
    groups: List[str] = field(default_factory=lambda: [])
    colors: Dict[str, str] = field(default_factory=lambda: {
        "WT": "blue", "KO": "red", "E2": "green", "E4": "orange"
    })
    ...
    replicate_offsets: Dict[str, int] = field(default_factory=lambda: {
        "B114": -35, "B115": 20, "B116": -55, "B117": 5
    })
    ...
    enable_thick_thin_analysis: bool = False
    width_threshold: int = 6
    ...
    def validate(self) -> None:
        ...
        # Validate replicate offsets match group names when using hierarchical structure
        if self.use_hierarchical_structure and not self.auto_detect_groups:
            # If groups are manually specified, check that offsets exist for each group
            missing_offsets = set(self.groups) - set(self.replicate_offsets.keys())
            if missing_offsets:
                raise ValueError(f"Missing replicate offsets for groups: {missing_offsets}. "
                              f"Ensure replicate_offsets in config.yaml uses the exact group names.")
        ...
        # Validate regression model configuration
        if self.use_regression_model:
            if not os.path.exists(self.regression_model_path):
                raise ValueError(f"Regression model file not found: {self.regression_model_path}")
            if not self.regression_model_path.endswith('.json'):
                raise ValueError(f"Regression model file must be a .json file: {self.regression_model_path}")
...
class ConfigManager:
    ...
    def generate_colors(conditions: List[str]) -> Dict[str, str]:
        """
        Generate distinct colors for conditions.
    ...
    def get_default_config() -> AnalysisConfig:
        ...
        config = AnalysisConfig(
            ...
            output_dir="/Users/charlessander/Desktop/patzke lab computing/Sudhof Lab/Axon Analysis Pipeline/Confocal_Axon_Analysis_Pipeline/Data_Output"
        )
        config.validate()
        return config
